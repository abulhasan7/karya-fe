...
var {register,login,updateProfile,getProfile,acceptProposal,postJob,getJob,getJobs, getJobsByStatus,updateStatus, enquiry}= require('../service/userService');
...
router.post('/enquiry', checkAuthenticationHeader,async function(req, res, next) {
  try{
  console.log(req.body);
  let resp = await enquiry({...req.body,user:req._id});
  console.log('resp is',resp);
  res.json({message:resp});
  }catch(error){
    console.log("error occurred",error);
